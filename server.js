...
class SudokuGenerator {
    ...
    generate(difficulty) {
        ...
        // Retirer des cases
        let attempts = difficulty === 'hard' ? 55 : (difficulty === 'medium' ? 45 : 30);
        while (attempts > 0) {
...
io.on('connection', (socket) => {
    ...
    socket.on('join_room', ({ roomCode, username }) => {
        ...
        const colors = ['#86efac', '#fca5a5', '#93c5fd', '#fde047', '#d8b4fe', '#fdba74'];
        ...
        socket.emit('joined_success', { roomCode, playerId: socket.id, mode: room.mode });
    });
    ...
    socket.on('update_progress', ({ roomCode, progress }) => {
        ...
        if (player) {
            ...
            io.to(roomCode).emit('progress_update', room.players.map(p => ({
                id: p.id,
                username: p.username,
                progress: p.progress
            })));
