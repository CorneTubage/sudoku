...
const app = {
    ...
    startSolo: (difficulty = 'medium') => {
        ...
        document.getElementById('mode-display').innerText = "Mode Solo (" + difficulty + ")";
    ...
    setLobbyDifficulty: (diff) => {
        if(app.roomCode) {
            socket.emit('change_difficulty', { roomCode: app.roomCode, difficulty: diff });
        }
    },
...
socket.on('update_lobby', (data) => {
    ...
    data.players.forEach(p => {
        ...
        li.className = "flex items-center gap-2 bg-slate-50 p-2 rounded justify-between";
        li.innerHTML = `
            <div class="flex items-center gap-2">
                <div class="w-3 h-3 rounded-full" style="background:${p.color}"></div> 
                <span class="font-bold text-slate-700">${p.username}</span>
            </div>
            ${data.hostId === p.id ? '<span class="text-xs bg-yellow-100 text-yellow-800 px-2 py-1 rounded font-bold">HÔTE</span>' : ''}
        `;
        list.appendChild(li);
    });
    ...
    if(iAmHost) {
        ...
        document.getElementById('host-difficulty-controls').classList.remove('hidden');
        ...
        document.querySelectorAll('.diff-btn').forEach(btn => {
            ...
            if(btn.dataset.diff === data.difficulty) btn.classList.add('ring-2', 'ring-slate-400');
        });
    } else {
        document.getElementById('host-controls').classList.add('hidden');
        document.getElementById('guest-waiting').classList.remove('hidden');
        document.getElementById('host-difficulty-controls').classList.add('hidden');
        document.getElementById('guest-difficulty-display').classList.remove('hidden');
        
        const labels = { easy: "Facile", medium: "Moyen", hard: "Difficile" };
        document.getElementById('guest-difficulty-display').innerText = labels[data.difficulty] || data.difficulty;
    }
});
...
socket.on('game_started', (data) => {
    ...
    let modeName = app.currentMode === 'speedrun' ? 'Course' : 'Guerre de Territoire';
    document.getElementById('mode-display').innerText = `Multijoueur : ${modeName}`;
...
socket.on('player_left_game', (data) => {
    ...
    notif.className = "text-xs text-red-500 font-bold bg-white p-1 rounded shadow mb-1";
...
socket.on('wrong_move', (data) => {
    // Visual feedback for wrong move in territory
...
function updateHudProgress(players) {
   ...
   players.sort((a,b) => b.progress - a.progress).forEach(p => {
       const div = document.createElement('div');
       const playerObj = game.players.find(pl => pl.id === p.id);
       if(playerObj) {
           div.className = "mb-1";
           div.innerHTML = `
              <div class="flex justify-between text-xs mb-1"><span style="color:${playerObj.color}">${p.username}</span> <span>${Math.round(p.progress)}%</span></div>
              <div class="h-1 bg-slate-200 rounded-full overflow-hidden">
                  <div class="h-full bg-green-500 transition-all duration-500" style="width: ${p.progress}%"></div>
              </div>
           `;
           hud.appendChild(div);
       }
...
class SudokuLogic {
  constructor() { this.grid = new Array(81).fill(0); }
  isValid(grid, row, col, num) {
      ...
      for(let i=0;i<3;i++)for(let j=0;j<3;j++) if(grid[(sr+i)*9+(sc+j)]===num)return false;
      return true;
  }
  fillGrid(grid) {
      for(let i=0;i<81;i++) {
  ...
  generate(difficulty) {
      ...
      this.fillBox(g,0,0); this.fillBox(g,3,3); this.fillBox(g,6,6);
      this.fillGrid(g);
      let s=[...g];
  ...
  fillBox(g,r,c){
      for(let i=0;i<3;i++)for(let j=0;j<3;j++){
          let n; do{n=Math.floor(Math.random()*9)+1}while(!this.isSafe(g,r,c,n));
          g[(r+i)*9+(c+j)]=n;
      }
  }
  isSafe(g,r,c,n){
      for(let i=0;i<3;i++)for(let j=0;j<3;j++)if(g[(r+i)*9+(c+j)]===n)return false;
      return true;
  }
}
...
class Game {
  constructor() {
    ...
    this.lastScores = null; // To refresh UI on disconnect
  ...
  moveSelection(key) {
      if(this.selectedCellIndex === null) { this.selectCell(0); return; }
  ...
  startTimer() {
      ...
      this.timerInterval = setInterval(() => {
          if(!this.isPaused) {
              this.timerSeconds++;
              document.getElementById("timer").innerText = this.formatTime(this.timerSeconds);
          }
      }, 1000);
  }
  ...
  resetBoard() {
      if (app.currentMode === 'territory') return alert("Impossible de reset en mode Territoire !");
  ...
  updateHighlights() {
      document.querySelectorAll('.cell').forEach(c => c.classList.remove('selected', 'highlighted', 'same-number', 'highlighted-box'));
      if (this.selectedCellIndex === null) return;

      const cell = document.querySelector(`.cell[data-index='${this.selectedCellIndex}']`);
      ...
      document.querySelectorAll('.cell').forEach(c => {
          ...
          // 3x3 Box (Correction Math)
          if(r >= boxStartRow && r < boxStartRow + 3 && co >= boxStartCol && co < boxStartCol + 3) {
              c.classList.add('highlighted'); // Ou 'highlighted-box' si vous voulez une couleur différente
          }

          // Same number
          if(val !== 0 && (this.board[idx] === val || this.initial[idx] === val)) c.classList.add('same-number');
      });
  }
  ...
  toggleNoteNumber(num) {
      if(num === 0) { this.notes[this.selectedCellIndex] = []; return; }
      if (!this.notes[this.selectedCellIndex]) this.notes[this.selectedCellIndex] = [];
  ...
  checkProgress() {
      ...
      socket.emit('update_progress', { roomCode: app.roomCode, progress: percent });
  }
}
